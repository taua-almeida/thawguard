...
The company OIDC provider must register the exact callback URI `${THAWGUARD_PUBLIC_URL}/settings/authentication/oidc/callback`; linking, Test sign-in, and company login all use this single URI. Test sign-in requests only the `openid email` scopes and verifies the saved client credentials, authorization-code flow, signed ID token, and a signed email with `email_verified` `true` whose domain exactly matches a saved allowed domain. A successful test leaves the connection disabled; it creates no identity, user, or Thawguard session and persists no email or claim material. Thawguard does not bundle a provider or imply that a live provider is available in local development.

Company login is Administrator-only in this first release. One existing enabled Administrator with a local password explicitly links their own company identity from Authentication settings, then explicitly enables company login; only that linked Administrator can sign in through the provider. There is no just-in-time user creation and no general-user admission: allowed domains and provider sign-in never create accounts or grant access by themselves. Local password sign-in and password recovery remain available throughout. Thawguard stores the linked identity's issuer, client, subject, and canonical verified provider email, but stores no provider tokens or raw claim payloads. Disabling company login or unlinking the identity immediately revokes every company sign-in session; local password sessions are unaffected.
...
- `/users` admin-only Users & Access directory with invitation creation and the active-invitation list, including replacing or cancelling an active invitation link; `/invitations/accept` anonymous invitation acceptance; `/users/{id}` manages account state, global Admin access, per-repository Viewer, Freezer, and Thaw-approver grants, and one-hour manual password-recovery links for other enabled local-password users

Admin is global installation management and can view every repository. Viewer, Freezer, and Thaw approver are granted per repository; any scoped grant permits reading that repository, while Freezer and Thaw approver permit only their matching actions. Admin does not imply either action role, including scheduled-freeze editing or Start Now. Inviting a person is the primary way to add someone: the invitee chooses their own password when accepting and starts with exactly the staged access. Local users created through the temporary fallback begin active with zero repository access and must replace their temporary password at first sign-in. If you bind beyond loopback after first-admin setup, keep Thawguard behind the network controls appropriate for your trusted team.
...
Invitations follow the same trusted-channel model. An Admin stages the identity, optional global Admin, and per-repository roles up front; Thawguard displays one single-use invitation link once, stores only its digest, and sends no email. The link is valid for up to seven days and stops working earlier if the invitation is cancelled or replaced, the authorizing Admin loses Admin access, or staged repository access changes. Staged access cannot be edited after the fact, but an Admin can replace an invitation's link: the previous link stops working immediately, the person, Admin setting, and staged repository access that still exists are preserved, and the new link is displayed once with a fresh seven-day expiry. A displayed link is never stored and can never be shown again. Accepting creates the account with the invitee's chosen password but never signs them in; they sign in afterwards through the ordinary login.
