...
The company OIDC provider must register the exact callback URI `${THAWGUARD_PUBLIC_URL}/settings/authentication/oidc/callback`. Test sign-in requests only the `openid email` scopes and verifies the saved client credentials, authorization-code flow, signed ID token, and a signed email with `email_verified` `true` whose domain exactly matches a saved allowed domain. Allowed domains are currently evaluated only by this Administrator Test sign-in; they do not yet admit users or grant access. A successful test leaves the connection Draft and disabled; it creates no identity, user, or Thawguard session and persists no email or claim material. Thawguard does not bundle a provider or imply that a live provider is available in local development.
