...
57. checks all three token values against rendered diagnostic pages after restart and after the terminal merge, relevant HTTP responses and redirects, Forgejo status API responses, captured Go test output, and both container logs without printing unsafe content; and
58. removes both containers and both named volumes on success or failure.
...
The disposable smoke covers the freeze/lift, active-cancel, immediate unique-head thaw, synchronized stale-head reevaluation, shared-head confirmation, and scheduled create/edit/pending-cancel/Start Now paths plus the setup-readiness, webhook, and token-failure P0 rows below. Add the remaining cases in this order:
