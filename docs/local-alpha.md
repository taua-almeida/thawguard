...
57. uses the first all-role E2E administrator's real `/users` form to create three independent single-role users — **E2E Admin Only**, **E2E Freezer**, and **E2E Thaw Approver** — then signs each in through a fresh cookie jar using the signed-CSRF `GET /login` and `POST /login` flow; Admin-only creates **E2E Viewer** through the same form, and Viewer signs in through a fourth isolated jar;
58. proves every single-role session receives HTTP 200 for the dashboard, repositories, active freezes, scheduled freezes, thaw requests, activity, status diagnostics, and webhook diagnostics; only Admin-only receives HTTP 200 for `/users`, while Freezer, Thaw approver, and Viewer receive exact HTTP 403 responses and never see **Users & Roles** navigation;
59. verifies rendered controls match the explicit role contract: Admin-only manages repositories/users and may edit or Start Now a pending schedule but cannot create/end/cancel active freezes, create/cancel schedules, or approve thaws; Freezer receives active-freeze and full schedule controls but no repository/user/thaw controls; Thaw approver receives only the thaw mutation form; Viewer receives read-only surfaces only;
60. sends valid-CSRF, otherwise mutation-capable wrong-role forms for every admin-only repository route (repository creation, managed-branch add/remove, webhook-secret and status-token rotation, readiness check, status verification, activation, deactivation, reconciliation, and recovery) and every admin-only user route (user creation, role update, disable, enable, and password reset), plus active-freeze create/end/cancel, scheduled create/edit/Start Now/cancel, and thaw approval; every request requires exact HTTP 403, while full stable admin `/users` and `/repositories` snapshots plus user/repository/freeze/schedule, webhook, status-result, desired-intent, publication-attempt, activity, session, and Forgejo-status evidence prove no denied mutation occurred;
61. has **E2E Freezer** create future fictional Schedule C on `release`, has **E2E Admin Only** edit it and use **Start Now**, and proves those three activity rows carry the exact authorized actors while completed Schedule A and cancelled Schedule B remain unchanged;
...
67. runs a final read-only observability phase over the real persisted activity feed, requires all 53 events to fit below its 100-event bound, and proves representative repository setup, credential, readiness, status verification, enforcement, recovery, synchronization, freeze/lift/cancel, schedule, planned-unfreeze, and single/shared-thaw rows retain curated action, target, outcome, and detail fields; exact actor evidence remains visible for **E2E Admin**, **E2E Admin Only**, **E2E Freezer**, **E2E Thaw Approver**, **Scheduler**, and the truthful **Unknown system actor**, with newest role-boundary evidence preceding early setup history;
...
The role-boundary phase uses four independent single-role local users rather than relying on the bootstrap account's all-role grant. It checks read access and rendered controls separately from valid-CSRF authorization probes, then compares exact persisted and Forgejo evidence to prove denied requests are side-effect free. The allowed sequence exercises the intentional Admin schedule exception — edit and **Start Now**, but not create or cancel — alongside Freezer schedule creation/active cancellation and Thaw-approver unique-head approval. Advancing one formerly shared PR also exercises the existing previous-head recomputation invariant: the other open PR stays on the old SHA with every prior status retained, followed by one expected refreshed success for its still-applicable thaw.
...
The disposable smoke covers freeze/lift, active cancellation, immediate and role-attributed unique-head thaws, synchronized stale-head reevaluation, shared-head confirmation, scheduled lifecycle and restart behavior, four single-role permission boundaries, setup readiness, webhook handling, token failure, and terminal audit/diagnostic evidence. All currently listed expansion rows are covered below:
...
| P2 (covered) | Viewer/freezer/thaw-approver/admin permissions | Four independent single-role sessions prove every normal-page read boundary, admin-only `/users` and setup controls, the Admin edit/Start Now schedule exception, exact 403 side-effect-free wrong-role probes, and correctly attributed Freezer/Thaw-approver actions. |
