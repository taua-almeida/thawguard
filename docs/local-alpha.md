...
For company OIDC, register `${THAWGUARD_PUBLIC_URL}/settings/authentication/oidc/callback` as the exact provider callback URI. Test sign-in requests only `openid email` and verifies the saved client credentials, authorization-code redirect, signed ID token, and a signed email with `email_verified` `true` in exactly one saved allowed domain. Saved allowed domains are currently evaluated only by this Administrator Test sign-in and do not yet admit users or grant access. Test sign-in does not create an identity, user, or Thawguard session, persists no email or claim material, and the connection remains Draft and disabled. The local stack includes no OIDC provider and does not imply that a live provider is available.
