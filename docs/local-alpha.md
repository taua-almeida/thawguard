...
For company OIDC, register `${THAWGUARD_PUBLIC_URL}/settings/authentication/oidc/callback` as the exact provider callback URI; linking, Test sign-in, and company login all share this single URI. Test sign-in requests only `openid email` and verifies the saved client credentials, authorization-code redirect, signed ID token, and a signed email with `email_verified` `true` in exactly one saved allowed domain. Test sign-in does not create an identity, user, or Thawguard session, persists no email or claim material, and leaves the connection disabled. The local stack includes no OIDC provider and does not imply that a live provider is available.

Company login is Administrator-only in this first release: one existing enabled Administrator with a local password links their own company identity, then explicitly enables company login, and only that linked Administrator can sign in through the provider. There is no just-in-time user creation and no general-user admission — allowed domains and provider sign-in never create accounts or grant access by themselves. Local password sign-in and password recovery remain available throughout; Thawguard stores the linked subject and canonical verified provider email, but stores no provider tokens or raw claim payloads. Disabling company login or unlinking the identity immediately revokes every company sign-in session.
